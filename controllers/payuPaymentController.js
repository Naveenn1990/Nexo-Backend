...
exports.initiatePayment = async (req, res) => {
  try {
    ...
    const paymentData = {
      ...
      email,
      phone,
      surl: `${process.env.BASE_URL || 'http://localhost:9088'}/api/payu/payment-success`,
      furl: `${process.env.BASE_URL || 'http://localhost:9088'}/api/payu/payment-failure`,
  ...
  try {
    ...
    if (!PAYU_CONFIG.skipHashVerification) {
      ...
      if (!isValid) {
        ...
        return res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:9088'}/partner/onboard?payment=failed&reason=invalid_hash`);
      }
    ...
    if (status === 'success') {
      res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:9088'}/partner/onboard?payment=success&txnid=${txnid}&payid=${mihpayid}`);
    } else {
      res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:9088'}/partner/onboard?payment=failed&reason=payment_failed`);
    }
  } catch (error) {
    console.error('❌ Payment callback handler error:', error);
    res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:9088'}/partner/onboard?payment=failed&reason=server_error`);
  }
};
